...
public class AdminModel {
	...
	public static ArrayList<Ticket> createReport() {
		String query = "SELECT T_ID, T_PRIORITY, T_CATEGORY, T_DESCRIPTION, "
				...
				+ "FROM TICKET, EMPLOYEE E1, EMPLOYEE E2 " + "WHERE TICKET.T_EMP_ID = E1.EMP_ID AND T_STATUS = 1 "
		...
		for (int i = 0; i < numOfRecords; i++) {
			...
			String status = (rs.get(i)[4] == null) ? TicketStatus.toStart()
					: (!(boolean) rs.get(i)[4]) ? TicketStatus.toFinish() : TicketStatus.finished();
	...
	public static boolean confirmRegistration(Registration registration) {
		String query = "BEGIN TRANSACTION " + "    INSERT INTO EMPLOYEE (EMP_USERNAME, EMP_PASSWORD, "
				...
				+ "    DELETE FROM REGISTRATION " + "    WHERE R_ID = " + registration.getId() + "COMMIT";
	...
	public static boolean updateFAQ(FAQ[] faq) {
		...
		try { // Query to add new entries
			CallableStatement cstmt = AuthenticationModel.conn.prepareCall("{call ADD_FAQ(?,?)}");
	...
	/**
	 * this function retrieve all the active users from the databse and returns an
	 * arraylist of all them. Stylianos
	 * 
	 * @return
	 */
...
///////////////////////////////////////////////////////////////
	...
	/**
	 * this function should retrieve all waiting to be approved users and return
	 * them as an arrayList of Users. idn what registration is. En ipame na exume
	 * mono to users je na eshi null fields?
	 * 
	 * @return
	 */
...
//				throw new SQLException("Approving Registration failed, no rows affected.");
	...
	/**
	 * This function removes the user from the waiting to be approved table. returns
	...
	public static FAQ[] searchFAQ(String s) {
		List<Object[]> results = new ArrayList<Object[]>();
		...
		for (int i = 0; i < faq.length; i++) {
			...
			faq[i] = UserModel.covertObjectArrayFAQ(temp);
		}
		return faq;
	...
	public static Policy[] searchPolicy(String s) {
		List<Object[]> results = new ArrayList<Object[]>();
		...
		Policy[] faq = new Policy[results.size()];
		for (int i = 0; i < faq.length; i++) {
			Object[] temp = results.get(i);

			faq[i] = UserModel.covertObjectArrayPolicy(temp);
		}
		return faq;
	...
	public static User2[] getAdmins() {
		List<Object[]> results = new ArrayList<Object[]>();

		String query = "SELECT *\r\n" + "FROM EMPLOYEE2 WHERE ISADMIN = 1 AND IS_SUPER_ADMIN = 0";
		results = getDataFromDB(query);

		User2[] users = new User2[results.size()];
		for (int i = 0; i < users.length; i++) {
			Object[] temp = results.get(i);

			users[i] = new User2((int) temp[0], (String) temp[1], (String) temp[1], (String) temp[1], (String) temp[1],
					(boolean) temp[1],(String) temp[1],(String) temp[1],(boolean) temp[1]);
		}
		return users;
	}
